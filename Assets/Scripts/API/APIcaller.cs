...
public class APIcaller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            DisableButton.gameObject.GetComponent<Button>().interactable = false;
        }
        GetSomeData();
    ...
    void GetSomeData()
    {
        ...
        Metrics metric = new Metrics(scaled_X, y / 2, z / 2,
            String.Format("User has a score of {0} for Problem Solving data obtained from TopCoder", x),
            String.Format("User has a score of {0} for competetive data obtained from TopCoder", y),
            String.Format("User has a score of {0} for persistence data obtained from TopCoder", z));


        PlayerPrefs.SetFloat("ProblemSolving", metric.problemSolving);
        PlayerPrefs.SetFloat("Competetive", metric.competetive);
        PlayerPrefs.SetFloat("Persistant", metric.persistant);

        PlayerPrefs.SetString("ProblemReason", metric.reasons[0]);
        PlayerPrefs.SetString("CompReason", metric.reasons[1]);
        PlayerPrefs.SetString("PersReason", metric.reasons[2]);
